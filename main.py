...
ROIs = st.multiselect("Choose the surface region", surface_atlas, surface_atlas[5:10])
if cofounder_type_code == 0:
    step = 30
    overlap = 0
    chart_surface = alt.Chart(df1,title=f'Cortical thickness at region against age').transform_bin(
    ...
    #     strokeWidth=0.5
            ...
            x=alt.X('value:T',title='Cortical thickness (normalized)',axis=None,scale=alt.Scale(domain=[0, 5])),
            # color='Z1:O',
            y=alt.Y(
                'density:Q',
                stack=None,
    ...
    #                 labelPadding=0,
        ...
        titleOrient='bottom', labelOrient='bottom',labelAnchor='start'
    )
...
#         scale=alt.Scale(domain=[30, 5], scheme='redyellowblue'))
    ...
    titleOrient='bottom', labelOrient='bottom',labelAngle=90,labelAnchor='start'
).configure_view(
    stroke=None
)
...
ROIs = st.multiselect("Choose the surface region", volume_atlas, volume_atlas[5:10])
if cofounder_type_code == 0:
    step = 30
    overlap = 0
    chart_volume = alt.Chart(df2,title=f'Cortical thickness at region against age').transform_bin(
            'Age', field='Z1',bin=alt.Bin(maxbins=2)
    ).transform_fold(
        ROIs,
        as_ = ['Cortical thickness', 'value'] #Xi: key value:value
...
# base_chart = alt.Chart(df1,width=30).mark_area(orient='horizontal').transform_fold(
...
#     as_=['value', 'density'],
# ).encode(
#         y=alt.Y('value:Q',title='Cortical thickness (normalized)'),
#         color=alt.Color('Z0:N', scale=alt.Scale(scheme='dark2'),),#legend=None
#         x=alt.X(
#             'density:Q',
#             stack=False,
...
# #         scale=alt.Scale(domain=[30, 5], scheme='redyellowblue'))
# ).transform_calculate(
#     density='datum.Z0==1.0?datum.density:datum.density*-1',
#     cat="datum.Xi + '-' + datum.Z0"
# ).configure_facet(
#     spacing=0
# ).configure_view(
#     stroke=NonROIs = st.multiselect("Choose the surface region", surface_atlas, surface_atlas[5:10])
