...
# Normalization before regression
	...
	N_test = 300
	X_test_true = np.random.rand(N_test,I)@np.diag(S)[:Rx,:Rx]@ V[:,:Rx].T 
	...
	X_test = X_test_true + Z_test@ZX*np.sqrt(np.linalg.norm(X_test_true, axis=0)/10**(SNR_ZX/10))
